...
function mergeObjects(objects) {
  return objects.reduce((acc, item) => {
    const entriesArrInItem = Object.entries(item);
    entriesArrInItem.forEach(([key, value]) => {
      const accItem = acc[key];
      if (accItem > 0) {
        acc[key] = accItem + value;
      } else {
        acc[key] = value;
      }
    });
...
function sellTickets(queue) {
  let result = queue[0] === 25;
  if (result) {
    queue.reduce((acc, elem) => {
      if (elem - 25 > acc) {
        result = false;
      }
      return acc + elem;
...
function Rectangle(width, height) {
  const rectangleObj = { width, height };
  function getArea() {
    return this.width * this.height;
  }
  rectangleObj.getArea = getArea;
  return rectangleObj;
}
...
function sortCitiesArray(arr) {
  const sortedByCities = arr.sort((a, b) => a.city.localeCompare(b.city));
  return sortedByCities.sort((a, b) => a.country.localeCompare(b.country));
}
